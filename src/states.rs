...
pub fn main_start(world: &mut World) {
...
pub fn main_update(world: &mut World) -> Trans<State> {
    let Game {
        restart, ..
...
pub fn paused_start(_: &mut World) {
    println!("Game Paused");
}

pub fn paused_update(world: &mut World) -> Trans<State> {
    let Game {
        ref mut pause, ..
    } = *world.write_resource::<Game>();
